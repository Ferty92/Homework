...
public class ExternalizeRouteService extends RouteService<City, Route<City>> {
    ...
    public ExternalizeRouteService(CachePathProvider pathProvider, boolean devMode) {
    ...
    public Route<City> getRoute(String from, String to) {
        ...
        if (hash.contains(name)) {
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(pathToFile))) {
                ...
                route = new ExternalizeRoute();
                ((ExternalizeRoute) route).readExternal(ois);
            ...
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(pathToFile))) {
                ...
                ((ExternalizeRoute) route).writeExternal(oos);
    ...
    protected City createCity(int id, String cityName, LocalDate foundDate, long numberOfInhabitants) {
        return new ExternalizeCity(id, cityName, foundDate, numberOfInhabitants);
    }

    @Override
    protected Route<City> createRoute(List<City> cities) {
        return new ExternalizeRoute(name, cities);
    }
}